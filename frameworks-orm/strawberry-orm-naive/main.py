#!/usr/bin/env python3
"""
Naive ORM Implementation: Strawberry GraphQL with SQLAlchemy 2.0 ORM
- Uses proper relationships and foreign keys
- Declarative relationships (one-to-many, many-to-one)
- Proper ORM session management
- INTENTIONALLY demonstrates N+1 query problems (lazy loading)
"""
...
# ============================================================================
# Naive Implementation: No DataLoaders (Causes N+1 Queries)
# ============================================================================
...
class CommentGQL:
    ...
    async def author(self) -> Optional["UserGQL"]:
        if not self.author_id:
            return None
        # NAIVE: Direct database query for each comment (N+1 problem!)
        async with async_session_maker() as session:
...
class PostGQL:
    ...
    async def author(self) -> Optional["UserGQL"]:
        if not self.author_id:
            return None
        # NAIVE: Direct database query for each post (N+1 problem!)
        async with async_session_maker() as session:
            user = await session.get(User, self.author_id)
            if user:
                return UserGQL(
    ...
    @strawberry.field
    async def comments(self) -> list[CommentGQL]:
        # NAIVE: Direct database query for each post (N+1 problem!)
        async with async_session_maker() as session:
            stmt = select(Comment).where(Comment.post_id == self.id)
...
class UserGQL:
    ...
    async def posts(self) -> list[PostGQL]:
        # NAIVE: Direct database query for each user (N+1 problem!)
        async with async_session_maker() as session:
            stmt = select(Post).where(Post.author_id == self.id)
            result = await session.execute(stmt)
...
# GraphQL router - No DataLoaders (naive implementation)
graphql_router = GraphQLRouter(Query)
